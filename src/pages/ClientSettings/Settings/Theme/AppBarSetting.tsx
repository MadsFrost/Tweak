...
const AppBarSetting = () => {
    ...
    const handleOnChangeBackground = (result: ColorResult) => {
        dispatch(setBackground({ color: result, type: 'appbar'}))
    }

    const handleOnChangeText = (result: ColorResult) => {
        dispatch(setText({ color: result, type: 'appbar'}))
    }
    const { background, text } = useAppSelector((state) => state.client.options.appBar.style)
